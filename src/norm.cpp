...
int
normalize(std::istream &istream, std::ostream &ostream, const ForeignLanguage language,
          const LatinOrthography orthography, const Diphthong diphthongs, const InitialCluster clusters
) {
    std::string line;
    while (std::getline(istream, line)) {
        ostream << normalizer(line, language, orthography, diphthongs, clusters) << std::endl;
        if (ostream.bad()) {
...
main(const int argc, char **argv) {
    ...
    "constraints of the specified orthography.",
    "norm"};
    app.option_defaults()->always_capture_default();

    std::filesystem::path input_param;
    app.add_option("-i,--input", input_param, "Input file or '-' to read from stdin")->check(CLI::ExistingFile);

    std::filesystem::path output_param;
    app.add_option("-o,--output", output_param, "Output file, otherwise stdout")->check(CLI::ExistingFile);

    const char *ABAKADA = {"abakada"};
    std::string_view ortho_param = ABAKADA;
    app.add_option("-O,--orthography", ortho_param)->check(CLI::IsMember({ABAKADA, "alpabetong"}));

    const char *SPANISH = {"spanish"};
    std::string_view lang_param = SPANISH;
    app.add_option("-l,--language", lang_param)->check(CLI::IsMember({SPANISH, "english"}));

    const char *REFORMED = {"reformed"};
    std::string_view diphthongs_param = REFORMED;
    app.add_option("-d,--diphthong", diphthongs_param)->check(CLI::IsMember({REFORMED, "traditional"}));

    std::string_view clusters_param = REFORMED;
    app.add_option("-c,--clusters", clusters_param)->check(CLI::IsMember({REFORMED, "traditional"}));
    ...
    if (const auto istream = get_input_stream(input_param);
        istream->good()) {
        if (const auto ostream = get_output_stream(output_param);
            ostream->good()) {
